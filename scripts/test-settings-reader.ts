...
assert.equal(writeSettingsKey("previewLines", 12), true, "a successful recovery reports that the setting was saved");
...
assert.equal(writeSettingsKey("previewLines", 12), false, "a failed backup reports that the setting was not saved");
...
assert.equal(writeSettingsKey("previewLines", 12), true, "a non-object settings file is recoverable like malformed JSON");
...
assert.equal(writeSettingsKey("spinnerColor", "after"), false, "an atomic write failure reports that the setting was not saved");
...
process.env.HOME = "";
try {
	assert.equal(writeSettingsKey("previewLines", 12), false, "an empty HOME reports that the setting was not saved");
} finally {
