...
Known Pi versions use the tested visual adapter directly. A newer version can also use it automatically when the core component, container, message, and spinner contracts pass runtime probes; each feature still requires its own capability and can fail native independently. If those core probes fail, Claudify enters the portable profile and shows a startup warning instead of silently looking stock. `PI_CLAUDIFY_PROFILE=portable` remains the explicit emergency fallback.

Claudify selects a runtime profile once per extension generation. `auto` is the default: validated releases and newer hosts that prove the required core contracts use the tested adapter; an unknown host whose probes fail uses the portable profile. Set `PI_CLAUDIFY_PROFILE=portable` for an emergency native-presentation pass-through that installs no private renderer patches or built-in overrides; collision-safe Cron and Ask tools remain available through documented public APIs and retain their individual kill switches. `PI_CLAUDIFY_PROFILE=tested-pi` selects the full adapter explicitly; `legacy` is accepted as a migration alias. A requested `certified-host` profile fails to portable mode until Pi exposes and Claudify implements the required public host contracts.

Built-in tool execution remains owned by Pi (or the pre-existing external extension); Claudify observes bounded call/result provenance and decorates compatible rows without re-registering `read`, `write`, `edit`, `bash`, `grep`, `find`, or `ls`. During the migration soak, set `PI_CLAUDIFY_NATIVE_EXECUTION=0` to restore the previous native-definition wrapper path for one generation. Exact compatibility tool switches continue to disable presentation regardless of execution ownership.
