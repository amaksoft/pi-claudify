...
- **Mutating pi structures needs a live smoke test before every release.** The assembled-render suites validate claudify's own output, not the format contracts of pi internals we write into — 2.3.0 shipped a crash because `theme.fgColors` stores ready-made ANSI escapes, not hex, and the accent override stored hex (rendered as literal `#B1B9F9`, widened lines, pi throws on overflow). Before `npm publish`, Berto opens a real pi session with the release checkout loaded (tmux capture method if the agent needs to observe) and eyeballs the changed surfaces.
