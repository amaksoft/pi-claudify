...
```bash
bun run test                       # full suite (16 assembled-render/handler suites)
...
**Do not launch pi under a headless pseudo-terminal** — it hangs forever without a real PTY. A detached **tmux** session gives pi a real PTY and is agent-drivable (`tmux send-keys` / `tmux capture-pane -e -p`) — use that for the pre-release live smoke test (see below) rather than a headless launch. Day-to-day behavior is still verified through the assembled-render/handler tests plus reasoned mechanism, as every suite does.
...
- **`extensions/index.ts`** (large) — the core. It monkey-patches pi's tool/message/container rendering, implements the diff renderer and the read-only-tool aggregation, wires OpenAI/MCP tool rows, and registers `/claudify` plus `/tui`. Most render logic lives here; the other `extensions/*.ts` are focused modules it imports.
...
- **`extensions/fullscreen-tui.ts`** — the session-local `/tui` fullscreen toggle. It owns the alternate screen, replaces an invisible marker above Pi's existing editor/footer with dynamic fill rows, and when transcript output overflows it owns a bounded viewport: Page Up/Page Down (fn+↑/fn+↓ on macOS) move about half a viewport, SGR wheel/trackpad events move by row, scrolled output shows Claude's `Jump to bottom: fn+↓ to scroll` hint, and submit snaps back to the live bottom. It composes with the renderer present at activation, restores the renderer, terminal mouse modes, `clearOnShrink`, normal buffer, and clean shell viewport on disable, reload, Ctrl+D, SIGTERM, or shutdown, and adds no setting, timer, persistence, or replacement transcript.
...
- **`extensions/footer.ts`** — the Claude Code-style statusline footer installed via pi's first-class `ctx.ui.setFooter`, plus `patchEditorBorderColor()`, which pins pi's input-box border to gray at every thinking level (Claude Code never tints it; bash mode's recolor is kept). The first segment names the **main repository** (resolved once per cwd via `git rev-parse --git-common-dir`, memoized), not `basename $PWD`, so a worktree does not repeat its branch name; model and context are active-model aware; OAuth quota is resolved only through pi's `ctx.modelRegistry`. Anthropic renders five-hour `Usage` plus weekly `Week`; OpenAI Codex renders `Week`. Validated quota values are cached in memory for 60 seconds. Tokens are never read from `auth.json` directly, persisted, logged, or cached. `footerUsageBar` replaces the old branch-local `footerContextBar`; `footerEffort` (default on) appends pi's live `ctx.getThinkingLevel()` to the model segment as `Fable 5 · high`, hidden when thinking is off. The border wrapper re-checks settings **when the colorizer runs** (every render), because pi reassigns `editor.borderColor` from `Theme.getThinkingBorderColor` on its own events, so instance patches don't hold. Capture/decisions: `docs/plans/2026-07-16-cc-input-box-footer.md`. Undocumented quota endpoint failures degrade to placeholders without affecting the footer or exposing credentials.
- **`extensions/spinner.ts`** — patches pi's `Loader` (glyphs, verb pool, colors). Reads settings each 250ms tick behind a 1s TTL plus a cross-module bust global (`bustSpinnerSettingsCache()`). The `spinnerShimmer` boolean setting (default on, Spinner section) animates the verb + glyph while the spinner runs — a one-way salmon→gold hue escalation, then a continuous loop alternating a sweep highlight with a whole-verb brightness pulse — via pure functions `shimmerBaseRgb`/`shimmerPhase`/`shimmerBreatheFactor`/`shimmerSweep`/`colorizeShimmerVerb`/`shimmerGlyphAnsi`, driving a faster ~200ms refresh while active. Suppressed when a custom `spinnerColor` is set (custom color wins over the shimmer's fixed palette).
...
- **Mutating pi structures needs a live smoke test before every release.** The assembled-render suites validate claudify's own output, not the format contracts of pi internals we write into — 2.3.0 shipped a crash because `theme.fgColors` stores ready-made ANSI escapes, not hex, and the accent override stored hex (rendered as literal `#B1B9F9`, widened lines, pi throws on overflow). Before `npm publish`, run a real pi session against the release checkout in a detached tmux session (an agent can drive this end-to-end, no human needed) and eyeball the changed surfaces. The smoke targets the local checkout only if **both** `~/.pi/agent/settings.json` and `<repo>/.pi/settings.json` have their `.packages[]` entry swapped from `npm:@owlburtoe/pi-claudify` to the checkout path — missing either silently smokes the published npm build instead.
- **pi's `Theme` reaches claudify as two object identities** — the instance and a forwarding Proxy, sharing one `fgColors` container. Keying per-theme state (e.g. an accent-restore snapshot) on the theme object itself gives each identity its own entry, so whichever arrives second sees claudify's own override already installed and misrecords it as the theme's original — permanently stranding the override. Key any such state on `theme.fgColors` instead (it forwards through the Proxy).
