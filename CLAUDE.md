...
- **`extensions/claudify-screen.ts`** — `ClaudifyScreen`, the full-screen `/claudify` TUI overlay (a Hub of Sections of setting rows). Row kinds: immediate-commit (boolean/enum/number), **text** (inline input sub-mode), **picker** (live-preview candidate list), and **verbs** (list editor). Guard against `mode !== "tui"`.
- **`extensions/settings.ts`** — `readSettings()` / `writeSettingsKey()`. Settings are **user-scope only** (`~/.pi/settings.json`); the project file is ignored (ADR 0004). Writes are atomic (tmp+rename). `normalizeAliases` maps legacy 1.x keys on both read and write (`spinnerVerbColor`→`spinnerColor`, `toolBackground: "border"`→`"outlines"`) — **keep it intact so 1.x settings keep loading.**
